...
class Seasons {
  ...
  next() {
    // ✨ implement
    const result = this.seasons[this.currSeason];
...
class Car {
  ...
  drive(distance) {
    // ✨ implement
    const fuelDistance = this.tank * this.mpg;
    if (distance <= fuelDistance) {
      this.odometer = this.odometer + distance;
      this.tank = this.tank - distance / this.mpg;
    } else {
      this.tank = 0;
      this.odometer = this.odometer + fuelDistance;
    }
  ...
  refuel(gallons) {
    // ✨ implement
    const tankLimit = this.tankSize - this.tank;
    if (gallons <= tankLimit) {
